...
function App() {
  ...
  const saveToSessionStorage = (data) => {
    try {
      sessionStorage.setItem('tradingJournalData', JSON.stringify({
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const allData = { profiles: profilesData, currentProfile: profilesData[0] || 'Profile 1', trades: {}, tags: [], strategies: [] };
  ...
  const handleProfileChange = async (profile) => {
    ...
    console.log(`Switching to profile: ${profile}, trades available:`, trades[profile]);
  ...
  const addTrade = (t) => setTrades((p) => ({ ...p, [currentProfile]: { ...(p[currentProfile] || {}), [t.date]: [...(p[currentProfile]?.[t.date] || []), t] } }));
  const updTrade = (d, i, t) => setTrades((p) => {
    const updated = { ...p };
    updated[currentProfile][d] = updated[currentProfile][d].map((x, j) => (j === i ? t : x));
    return updated;
  });
  const delTrade = (d, i) => setTrades((p) => {
    const updated = { ...p };
    updated[currentProfile][d] = updated[currentProfile][d].filter((_, j) => j !== i);
