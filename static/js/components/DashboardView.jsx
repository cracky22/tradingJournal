...
function DashboardView({
  ...
  useEffect(() => {
    ...
    const totalProfitLoss = trades.reduce(
      (sum, trade) => sum + (trade.profitLossDollar || 0),
      0
    );
    ...
    const winningTrades = trades.filter(
      (trade) => (trade.profitLossDollar || 0) > 0
    );
    const avgWin =
      winningTrades.length > 0
        ? winningTrades.reduce(
            (sum, trade) => sum + trade.profitLossDollar,
            0
    ...
    const avgLoss =
      losingTrades.length > 0
        ? losingTrades.reduce((sum, trade) => sum + trade.profitLossDollar, 0) /
          losingTrades.length
        : 0;
    ...
    const longTrades = trades.filter(
      (trade) => trade.direction === "Long" || !trade.direction
    );
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: (context) =>
                  `Profit: $${context.raw.y.toFixed(2)}, Date: ${
                    context.raw.x
                  }`,
              },
            },
            ...
            tooltip: {
              callbacks: {
                label: (context) =>
                  `Duration: ${
                    context.raw.x
                  } min, Profit: $${context.raw.y.toFixed(2)}`,
              },
            },
          },
        },
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {/* Summary Statistics in groups of 3 */}
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-gray-800 p-4 rounded-lg shadow">
          ...
          <p
            className="text-2xl"
            style={{
              color: stats.avgProfitPerTrade >= 0 ? "#22c55e" : "#ef4444",
            }}
          >
        ...
        <div className="bg-gray-800 p-4 rounded-lg shadow">
          ...
          <p className="text-2xl text-white">
            ${Math.abs(stats.maxLossTrade).toFixed(2)}
          </p>
        </div>
        <div className="bg-gray-800 p-4 rounded-lg shadow">
